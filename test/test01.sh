...
echo "testing use of gtopo30 DEM tiles..."
...
if (test -r "$dem_file" && test -r "$dem_hdr"); then
  ...
  echo "you need the GTOPO30 test files \"$dem_file\" and \"$dem_hdr\" to run this test.  You can get them at http://http://www1.gsi.go.jp/geowww/globalmap-gsi/gtopo30/w180n90.html"
exit 0
fi
...
../src/puff -volc spurr -eruptDate "2006 07 25 06:00" -model gfs -dem gtopo30:4 -runHours 6 -rcfile ../etc/puffrc > /dev/null 2>$error_file 

if test -s $error_file; then
  exit 1
fi

rm ./200607251200_ash.cdf
rm $error_file
exit 0
