...
echo "testing ashdump.pl functionality..."

thisdir=`pwd`;
PUFFHOME=$thisdir/..
export PUFFHOME

#test for working ashdump.pl
../src/perl/ashdump.pl --version > /dev/null 2>$error_file
if  test $?; then
	echo "ashdump.pl not functional, skipping test. See test05.err for why."
	exit
fi
