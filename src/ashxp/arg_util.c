...
#define DEBUG(x,y) printf(x,y);
	...
	exit(0);
			...
			if (arguments.magnify<-10 || arguments.magnify>10)
			{
				puts("Invalid magnify value. Must be -10<magnify<+10. Resetting to zero.");
				arguments.magnify=0;
			}
      break;
    case 'o':
...
void show_help() {
  ...
  puts("--latlon=y1/y2/x1/x1  -l y1/y2/x1/x1   set bounding box");
	puts("--label=X,Y,STRING    -L X,Y,STRING    put STRING at (X,Y) degrees lat/lon");    
