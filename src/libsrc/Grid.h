...
protected:
    ...
    enum {UNKNOWN, REGIONAL, GLOBAL} coverage;
		enum {IS_PROJ_GRID, NOT_PROJ_GRID, UNDEF_PROJ_GRID} projectionGrid;
...
//    int dim_expand(ID, float start, float end);
    ...
    float nnint(float xx, float yy, float zz, float tt);
		float nnint(float xx, float yy, float zz, float tt, int i, int j, int k, int l);

		void indices(float x, float y, float z, float t, int &i, int&j, int &k, int &l);
