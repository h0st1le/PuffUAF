...
void Grid::get_attributes(NcVar *vp, int idx) {
	bool unpacked_valid_range_set = false;
  // retrieve attributes
  ...
  for (int i = 0; i< natt; i++) {
    ...
    if ((strcmp(attname,"valid_range") == 0 ) && !unpacked_valid_range_set)
    {
      if (att->num_vals() != 2 ) 
      {
    ...
    }  else if (strcmp(attname,"unpacked_valid_range") == 0) {  
      if (att->num_vals() != 2 ) 
      {
        std::cerr << "WARNING: unpacked_valid_range does not contain 2 values as expected\n"; 
      } else {
        fgData[idx].range[0]=att->as_float(0);
        fgData[idx].range[1]=att->as_float(1);
				unpacked_valid_range_set = true;
      }
...
//  utScan("hours since 1970-1-1", &time_t_unit_sct);
	utScan("seconds since 1970-1-1", &time_t_unit_sct);
  // recheck attribute
...
//  const double offset = (reftime_unit_sct.origin*reftime_unit_sct.factor) - (time_t_unit_sct.origin*time_t_unit_sct.factor);
	...
	// get the slope and intercept for converting between these two units
	// currently slope is not used and it is assumed all units are in "hours"
	// that should be generalized at some point in the future. TODO

	double slope, intr;
	utConvert(&reftime_unit_sct, &time_t_unit_sct, &slope, &intr);
	const double offset = intr;
