...
float ran1(int &idum) {
	...
	static int iff=0;
	int j;
#pragma omp threadprivate (ix1, ix2, ix3, iff, r)
...
#pragma omp threadprivate(iset, gset)
        ...
        if  (iset == 0) {
                ...
                gset=v1*fac;
                iset=1;
//#pragma omp critical (omp_gasdev)
//{
//	std::cout << omp_get_thread_num() << ": " << v2*fac << std::endl;
//}
                return v2*fac;
        } else {
                iset=0;
...
void init_seed(int& iseed, int set) {
    ...
    if (set) { tmpseed = set; }
		...
		// give each thread a different seed
		tmpseed=tmpseed+omp_get_thread_num();
    if (tmpseed > 1000 || tmpseed < -1000) {
	std::cout << "init_seed() WARNING: out of bounds,  setting iseed = 123\n";
	iseed = 123;
    }
    else {
	int tmpdum = tmpseed;
