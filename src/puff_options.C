...
//      argument.opath  = strdup(optarg);
    ...
    case PHIDIST:
			if (strlen(optarg)==0)
				{ std::cout << "phiDist requires an argument, ignoring\n";break;}
    ...
    case VERBOSE:
      if ( (optarg) && strlen(optarg) > 0 ) {
        if (toupper(optarg[0]) == 70) puff_verbose = false;
 	else if (toupper(optarg[0]) == 84) puff_verbose = true; 
 	else 
 	  std::cout << "unrecognized boolean option -verbose=" << optarg << std::endl;
         }  // if ( (optarg) && strlen(optarg) > 0 )
      else { puff_verbose = true; }
      break; 
    case PUFF_VERSION:
...
void show_help() {
  ...
  std::cout << "  -saveHours    value      (float)\n";
	...
	std::cout << "  -seed         value      (-1000<int<1000)\n";
  std::cout << "  -shiftWest\n";
