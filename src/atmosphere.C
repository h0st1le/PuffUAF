...
//////////////////////////////////////////////////////////////////////////
Particle Atmosphere::speed (float time, Particle *p) {
	int i, j, k, l;
	Particle d;

	U.indices(time, (*p).z, (*p).y, (*p).x, i, j, k, l);
	d.x = U.nnint(time, (*p).z, (*p).y, (*p).x, i, j, k, l);
	d.y = V.nnint(time, (*p).z, (*p).y, (*p).x, i, j, k, l);
	d.z = W.nnint(time, (*p).z, (*p).y, (*p).x, i, j, k, l);
	return d;
}
...
{
	if (argument.noFallout == true) return 0;
...
//   }
  ...
  // now read in temperature data if it is available and necessary
	...
	{
		VERBOSE("Loading temperature data");
    std::string Tfile;
    if ( !argument.fileT ) 
    {
...
// now set W's coverage, but maybe it should just be U and V's, right?
  ...
  if (puff_verbose) W.display (INFO);
...
int Atmosphere::read_uni (Grid & uni, std::string *filename)
{
  if (filename->length() == 0) 
	{
    std::cerr << "ERROR: no data found in file:\"" << filename << "\"\n";
    return PUFF_ERROR;
  }
  ...
  if (puff_verbose) uni.display (INFO);
