...
double Dem::elevation(double lat, double lon, maparam* proj_grid) 
{
  ...
  int xsize = tile[idx].ncols;
  int ysize = tile[idx].nrows;
...
#pragma omp critical
{
  ...
  if (type == DTED0) {
    e[0] = tile[idx].data[(i+1)*ysize-j];
    e[1] = tile[idx].data[(i+2)*ysize-j];
    e[2] = tile[idx].data[(i+2)*ysize-j+1];
    e[3] = tile[idx].data[(i+1)*ysize-j-1];
    }
...
int Dem::setResolution(int res) {
  ...
  int factor = (int)pow((double)2,(double)res);
