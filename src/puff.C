...
#ifndef VERBOSE
#define VERBOSE(x) if (puff_verbose) std::cout << x << std::endl;
#endif 
...
inline void meter2cart (PUFFdouble &dx, PUFFdouble &dy, PUFFdouble &y)
{
...
void (*meter2grid) (PUFFdouble &dx, PUFFdouble &dy, PUFFdouble &y);
...
#ifdef _OPENMP
	VERBOSE("Using OpenMP with " << omp_get_num_procs() << " processors");
#endif
...
{
#ifdef PUFF_MTWISTER
	VERBOSE("using Mersenne Twister PRGN");
	prng = new std::mt19937;
	prng->seed(rdtsc()+omp_get_thread_num());
#else
// iseed is threadprivate already
  init_seed (iseed, argument.seed);
#pragma omp critical
{  VERBOSE("Random number seed = " << iseed << std::flush); }
#endif  //PUFF_MTWISTER
} //omp parallel
  ...
  do {
    ...
    for (clock = eruptDate_t; 
         clock <= endDate_t && !EarlyEndOfSimulation; 
	 clock += dtMins_t) 
    {
      ...
      diffHrs = secs2hrs * (clock - reftime_t);
...
#ifdef PUFF_MTWISTER
#pragma omp parallel private (i) firstprivate (ch, dr, normDist) default(shared)
#else
#pragma omp parallel private (i) firstprivate (ch, dr) default(shared)
#endif //PUFF_MTWISTER
{
...
#pragma omp for
				...
				)
	{
...
#ifdef PUFF_MTWISTER
			dr.x = dtMins_t * ch * normDist(*prng);
			dr.y = dtMins_t * ch * normDist(*prng);
			dr.z = dtMins_t * cv * normDist(*prng);
#else
	    ...
	    ash.r[i].dif_x += fabs (dr.x);
	    ash.r[i].dif_y += fabs (dr.y);
	    ash.r[i].dif_z += fabs (dr.z);
#endif
	    // Advection:
 			dr = dr + (atm->speed(diffHrs, &ash.r[i]))*dtMins_t;
//			dr.x += dtMins_t * atm->xSpeed(diffHrs, &ash.r[i]);
//			dr.y += dtMins_t * atm->ySpeed(diffHrs, &ash.r[i]);
//			dr.z += dtMins_t * atm->zSpeed(diffHrs, &ash.r[i]);

	    // Fallout:
	    dr.z += atm->fallVelocity(diffHrs, &ash.r[i]) * dtMins_t;


#ifdef PUFF_STATISTICS
	    // units for adv_x depend on input data.  If atm velocity is m/s
	    // and dtMins_t is seconds, than adv_x is in meters
            ash.r[i].adv_x += fabs(dtMins_t * atm->xSpeed(diffHrs, &ash.r[i]) );
            ash.r[i].adv_y += fabs(dtMins_t * atm->ySpeed(diffHrs, &ash.r[i]) );
            ash.r[i].adv_z += fabs(dtMins_t * atm->zSpeed(diffHrs, &ash.r[i]) );
#endif
	    // Move to grid:
...
//    int multE = (strcmp (argument.volc, "none")); 
    bool multE = (strcmp (argument.volc, "none")); 
